...
# Reference audio lives in VOICES_DIR: <name>.<ext> (wav/mp3/m4a/flac/ogg/opus)
# plus optional <name>.txt (exact transcript of the reference clip; Whisper
# auto-transcribes if absent).

VOICES_DIR = os.getenv("VOICE_CLONES_DIR", "/app/models/voices")
_AUDIO_EXTS = (".wav", ".mp3", ".m4a", ".flac", ".ogg", ".opus")


def _clone_audio(name: str):
...
def resolve_clone(name: str):
    audio = _clone_audio(name)
    if audio is None:
        raise HTTPException(404, f"Voice clone '{name}' not found in {VOICES_DIR}")
...
def _safe_name(name: str) -> str:
    ...
    if not name or not all(c.isalnum() or c in "-_" for c in name):
        raise HTTPException(400, "Voice name must be alphanumeric/-/_ (e.g. 'justus')")
    return name
    ...
    if os.path.isdir(VOICES_DIR):
        for f in sorted(os.listdir(VOICES_DIR)):
            ...
            if ext.lower() in _AUDIO_EXTS:
                txt = os.path.join(VOICES_DIR, base + ".txt")
                clones.append({
                    "name": base,
                    "audio": f,
                    "ref_text": open(txt, encoding="utf-8").read().strip() if os.path.isfile(txt) else None,
                    "size_bytes": os.path.getsize(os.path.join(VOICES_DIR, f)),
                })
    return {"clones": clones, "dir": VOICES_DIR}


@app.post("/v1/audio/clones", tags=["Voice Clones"])
async def register_clone(
    name: str = Form(..., description="Voice name (alphanumeric/-/_), e.g. 'justus'"),
    ref_audio: UploadFile = File(..., description="Reference audio (WAV/MP3/M4A), 3-10 s recommended"),
    ref_text: Optional[str] = Form(None, description="Exact transcript of the reference audio"),
):
    """Register a named voice clone for use via voice="clone:<name>" on /v1/audio/speech."""
    n = _safe_name(name)
    os.makedirs(VOICES_DIR, exist_ok=True)
    raw = await ref_audio.read()
    if not raw:
        raise HTTPException(400, "Empty reference audio")
    ...
    if ref_text and ref_text.strip():
        with open(os.path.join(VOICES_DIR, n + ".txt"), "w", encoding="utf-8") as f:
            f.write(ref_text.strip())
    return {"success": True, "name": n, "bytes": len(raw), "ref_text": bool(ref_text)}
...
async def openai_tts(req: SpeechRequest):
    ...
    **Priority**: `duration` > `speed`.
    ...
    instruct = resolve_instruct(req.voice) if not (req.voice or "").startswith("clone:") else None
    clone_ref = resolve_clone(req.voice[6:].strip()) if (req.voice or "").startswith("clone:") else None
    t0 = time.time()
    try:
        kwargs: dict = {
            "text":                  req.input,
